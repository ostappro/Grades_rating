...
const put: APIResponder<UserResults> = async (req) => {
    ...
    const user = { id, email, grades, name } // Sanitizing any additional fields that may come in from the request
    await putGrade(user)

    // I'm in doubt whether i should return results on this request
